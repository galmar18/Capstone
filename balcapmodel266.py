# -*- coding: utf-8 -*-
"""
Created on Wed Jun 26 22:40:33 2019
...
data = pd.read_csv('C:\\Users\\Rachel\\Documents\\Capstone Project\\final12_246.csv', header=0)
...
clfRFmd2 = RandomForestClassifier(n_estimators=50, max_depth=100, random_state=None)
clfRFmd2.fit(x_train, y_train) 
y_test_pred_RFmd2=clfRFmd2.predict(x_test)

print ('Random Forest MaxDepth2: Macro Precision, recall, f1-score')
print ( precision_recall_fscore_support(y_test, y_test_pred_RFmd2, average='macro'))
print ('Random Forest MaxDepth2: Micro Precision, recall, f1-score')
print (precision_recall_fscore_support(y_test, y_test_pred_RFmd2, average='micro'))
print ('\n')
...
fprANN, tprANN, thresholdsANN = roc_curve(y_test, pr_y_test_pred_ANN2[:,1])
...
# So I will keep now only the 55 principal components (90% variance) when i create the PCA object
...
#PCA ANN2 Classifier - parameters (100,100)
clfANN2PCA = MLPClassifier(solver='adam', activation='relu',
                    batch_size=10, tol=1e-5,
                     hidden_layer_sizes=(100,100), random_state=1, max_iter=100000, verbose=50)

clfANN2PCA.fit(x_pca_train, y_pca_train)
y_test_pred_ANN2PCA = clfANN2PCA.predict(x_pca_test)

print ('ANN2PCA: Macro Precision, recall, f1-score')
print ( precision_recall_fscore_support(y_pca_test, y_test_pred_ANN2PCA, average='macro'))
print ('ANN2PCA: Micro Precision, recall, f1-score')
print (precision_recall_fscore_support(y_pca_test, y_test_pred_ANN2PCA, average='micro'))
print ('\n')
...
pr_y_test_pred_ANN2PCA=clfANN2PCA.predict_proba(x_pca_test)
...
fprANNPCA, tprANNPCA, thresholdsANNPCA = roc_curve(y_pca_test, pr_y_test_pred_ANN2PCA[:,1])
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Receiver Operating Characteristic PCA Balanced Data')
plt.legend(loc="lower right")
plt.show()
